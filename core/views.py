    ...
    if user.is_superuser or user.role == 'admin':
        context.update({
            ...
            'out_of_stock_items': InventoryItem.objects.filter(
                quantity__lte=0
            ).select_related('part', 'location')[:10]
        })
    ...
    # Procurement Officer Dashboard
        ...
        context.update({
            ...
            'recent_transactions': StockTransaction.objects.filter(
                performed_by=user
            ).select_related('inventory_item__part').order_by('-timestamp')[:5]
        })

    # Technician Dashboard
    elif user.role == 'technician':
        # Assuming you have a Task model or WorkOrder assigned to technicians
        context.update({
