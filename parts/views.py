...
class PartListView(LoginRequiredMixin, RoleRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Part.objects.select_related('category', 'brand').annotate(
            stock_quantity=Sum('inventoryitem__quantity', default=0)
        )
        ...
        if search:
            queryset = queryset.filter(
                Q(name__icontains=search) | Q(part_number__icontains=search)
            )
        if category:
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_stock'] = sum(part.stock_quantity for part in context['parts'])
        return context
